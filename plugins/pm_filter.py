...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/+jzw8ChsirtljZDdl>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=GRP_LNK)]]))
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/+jzw8ChsirtljZDdl")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    # don't change anything without contacting me @kingvj01
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/UHDOWNERBOT")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def advantage_spell_chok(client, name, msg, reply_msg, UHD_search):
    mv_id = msg.id
    mv_rqst = name
    ...
    if AI_SPELL_CHECK == True and UHD_search == True:
        UHD_search_new = False
        UHD_ai_msg = await reply_msg.edit_text("<b><i>Advance Ai Try To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
        ...
        for techvj in movienamelist:
            ...
            if mv_rqst.startswith(techvj[0]):
                await auto_filter(client, techvj, msg, reply_msg, UHD_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
